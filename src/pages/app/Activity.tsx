              ...
              <Card className="p-2">
                <div className="relative pl-2">
                  {items.map((a, i) => {
                    ...
                    return (
                      <div key={a.id} className="flex items-start gap-3 p-2.5 rounded-2xl hover:bg-overlay/[0.05] transition">
                        <div className="relative shrink-0">
                          ...
                          <span className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-white grid place-items-center text-brand-600 dark:text-brand-400 shadow-glass-sm">{ICONS[a.icon] ?? <ActivityIcon className="w-3 h-3" />}</span>
                          {i < items.length - 1 && <span className="absolute left-1/2 top-11 -translate-x-1/2 w-px h-[calc(100%-1rem)] bg-overlay/[0.08]" />}
                        </div>
